...
class Seed {
  ...
  drawText() {
    ...
    const fontSize = 11;
    ctx.font = `${fontSize}px "Tajawal", sans-serif`;
    ctx.direction = "rtl";
    ctx.textAlign = "right";
    ...
    const textY = 3;
    const lineY = 18;
    const textX = -(heartHalfW + gap);
    const lineEndX = textX - textWidth - 8;

    ctx.beginPath();
    ctx.moveTo(-heartHalfW + 2, 0);
    ctx.lineTo(-heartHalfW - 10, lineY - 2);
  ...
  clear() {
    ...
    const w = 26 * scale;
    const h = 26 * scale;
    const labelW = (46 + textWidth) * scale;
    ctx.clearRect(point.x - labelW, point.y - h, labelW + w * 2, h * 4);
  }
